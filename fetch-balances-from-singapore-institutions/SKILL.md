...
## Handling Legacy Frame-Based Architectures

Some Singapore bank websites use legacy frame-based layouts (framesets with nested iframes) from the pre-2010 era. Standard `document.body.innerText` returns empty or navigation-only content because the actual data lives in nested frames.
...
If `snapshot -c` returns only navigation text or empty content, check for frames:
...
```bash
# Find frame containing specific text (e.g., "Balance")
...
When DOM access is blocked or too complex:
...
```bash
# Find all currency amounts in the page
npx agent-browser eval "window.frames['user_area'].document.body.innerHTML.match(/\\d{1,3}(,\\d{3})*\\.\\d{2}/g)"

# Search for SGD amounts with context
npx agent-browser eval "window.frames['user_area'].document.body.innerHTML.match(/SGD[^<]*\\d+[,.]\\d+/g)"
```

#### Network Interception (Advanced)

For sites with complex DOM but clean APIs, intercept XHR responses:

```bash
# This requires Playwright/Puppeteer directly, not agent-browser CLI
# Monitor network tab in DevTools to find API endpoints
# Then call those endpoints directly if authenticated
```

### Troubleshooting Frame Access
